...
namespace Balltze::HEK::TagDefinitions { 
	enum WeaponSecondaryTriggerMode : std::uint16_t {
		WEAPON_SECONDARY_TRIGGER_MODE_NORMAL = 0,
	...
	enum WeaponFunctionIn : std::uint16_t {
		WEAPON_FUNCTION_IN_NONE = 0,
	...
	enum WeaponMovementPenalized : std::uint16_t {
	...
	enum WeaponPredictionType : std::uint16_t {
		WEAPON_PREDICTION_TYPE_NONE = 0,
		WEAPON_PREDICTION_TYPE_CONTINUOUS,
		WEAPON_PREDICTION_TYPE_INSTANT,
	};

	enum WeaponOverchargedAction : std::uint16_t {
		WEAPON_OVERCHARGED_ACTION_NONE = 0,
		WEAPON_OVERCHARGED_ACTION_EXPLODE,
		WEAPON_OVERCHARGED_ACTION_DISCHARGE,
	};

	enum WeaponDistributionFunction : std::uint16_t {
		WEAPON_DISTRIBUTION_FUNCTION_POINT = 0,
